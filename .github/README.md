...
Here’s the circle diagram of the Architecture (`internal/` directory):
...
All the code, as well as unit tests, is located in the `internal` directory. Note that the unit tests are written only for Application layer.

Integration tests are located in the `test` folder. Note, that there are tests for the Entity and Service layers.
