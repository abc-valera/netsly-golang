...
func (pm passwordMaker) HashPassword(password string) (string, error) {
...
func (pm passwordMaker) CheckPassword(password, encodedHash string) error {
	// Extract the parameters, salt and derived key from the encoded password hash.
	salt, hash, err := decodeHash(encodedHash)
	if err != nil {
		return codeerr.NewInternal("CheckPassword", err)
	}
