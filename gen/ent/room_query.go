...
type RoomQuery struct {
	...
	withMembers  *RoomMemberQuery
	withMessages *RoomMessageQuery
...
func (rq *RoomQuery) QueryMembers() *RoomMemberQuery {
...
func (rq *RoomQuery) QueryMessages() *RoomMessageQuery {
	query := (&RoomMessageClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) WithMembers(opts ...func(*RoomMemberQuery)) *RoomQuery {
	query := (&RoomMemberClient{config: rq.config}).Query()
...
func (rq *RoomQuery) WithMessages(opts ...func(*RoomMessageQuery)) *RoomQuery {
	query := (&RoomMessageClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withMembers; query != nil {
		if err := rq.loadMembers(ctx, query, nodes,
			func(n *Room) { n.Edges.Members = []*RoomMember{} },
			func(n *Room, e *RoomMember) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withMessages; query != nil {
		if err := rq.loadMessages(ctx, query, nodes,
			func(n *Room) { n.Edges.Messages = []*RoomMessage{} },
			func(n *Room, e *RoomMessage) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadMembers(ctx context.Context, query *RoomMemberQuery, nodes []*Room, init func(*Room), assign func(*Room, *RoomMember)) error {
	...
	for _, n := range neighbors {
		fk := n.room_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "room_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (rq *RoomQuery) loadMessages(ctx context.Context, query *RoomMessageQuery, nodes []*Room, init func(*Room), assign func(*Room, *RoomMessage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Room)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
