...
func testLikesExists(t *testing.T) {
	...
	e, err := LikeExists(ctx, tx, o.UserID, o.JokeID)
	if err != nil {
...
func testLikesFind(t *testing.T) {
	...
	likeFound, err := FindLike(ctx, tx, o.UserID, o.JokeID)
	if err != nil {
		t.Error(err)
	}
...
func testLikeToOneJokeUsingJoke(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadJoke(ctx, tx, false, (*[]*Like)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Joke == nil {
...
func testLikeToOneSetOpJokeUsingJoke(t *testing.T) {
	var err error
	...
	for i, x := range []*Joke{&b, &c} {
		...
		if exists, err := LikeExists(ctx, tx, a.UserID, a.JokeID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	likeDBTypes = map[string]string{`CreatedAt`: `timestamp without time zone`, `UserID`: `uuid`, `JokeID`: `uuid`}
	_           = bytes.MinRead
)
