...
var LikeWhere = struct {
	...
	DeletedAt: whereHelpertime_Time{field: "\"likes\".\"deleted_at\""},
...
var (
	likeAllColumns            = []string{"created_at", "deleted_at", "user_id", "joke_id"}
	likeColumnsWithoutDefault = []string{"created_at", "deleted_at", "user_id", "joke_id"}
...
func FindLike(ctx context.Context, exec boil.ContextExecutor, userID string, jokeID string, selectCols ...string) (*Like, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"likes\" where \"user_id\"=? AND \"joke_id\"=?", sel,
	)
...
func LikeExists(ctx context.Context, exec boil.ContextExecutor, userID string, jokeID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"likes\" where \"user_id\"=? AND \"joke_id\"=? limit 1)"
