...
var RoomMessageTableColumns = struct {
	...
	CreatedAt: "RoomMessage.created_at",
	UserID:    "RoomMessage.user_id",
...
var RoomMessageWhere = struct {
	...
	ID:        whereHelperstring{field: "\"RoomMessage\".\"id\""},
	Text:      whereHelperstring{field: "\"RoomMessage\".\"text\""},
	CreatedAt: whereHelpertime_Time{field: "\"RoomMessage\".\"created_at\""},
	UserID:    whereHelperstring{field: "\"RoomMessage\".\"user_id\""},
	RoomID:    whereHelperstring{field: "\"RoomMessage\".\"room_id\""},
}
...
var (
	roomMessageAllColumns            = []string{"id", "text", "created_at", "user_id", "room_id"}
	roomMessageColumnsWithoutDefault = []string{"id", "text", "created_at", "user_id", "room_id"}
...
func (q roomMessageQuery) One(ctx context.Context, exec boil.ContextExecutor) (*RoomMessage, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "sqlboiler: failed to execute a one query for RoomMessage")
	}
...
func (q roomMessageQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to count RoomMessage rows")
	}
...
func (q roomMessageQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "sqlboiler: failed to check if RoomMessage exists")
	}
...
func (roomMessageL) LoadRoom(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoomMessage interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`Room`),
		qm.WhereIn(`Room.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for Room")
	}
	...
	if singular {
		...
		foreign.R.RoomRoomMessages = append(foreign.R.RoomRoomMessages, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.RoomID == foreign.ID {
				...
				foreign.R.RoomRoomMessages = append(foreign.R.RoomRoomMessages, local)
				break
			}
		}
	}
...
func (roomMessageL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoomMessage interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.UserRoomMessages = append(foreign.R.UserRoomMessages, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.UserRoomMessages = append(foreign.R.UserRoomMessages, local)
				break
			}
		}
	}
...
// Adds o to related.R.RoomRoomMessages.
...
func (o *RoomMessage) SetRoom(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Room) error {
	...
	if related.R == nil {
		related.R = &roomR{
			RoomRoomMessages: RoomMessageSlice{o},
		}
	} else {
		related.R.RoomRoomMessages = append(related.R.RoomRoomMessages, o)
	}
...
// Adds o to related.R.UserRoomMessages.
// Uses the global database handle.
...
func (o *RoomMessage) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	if related.R == nil {
		related.R = &userR{
			UserRoomMessages: RoomMessageSlice{o},
		}
	} else {
		related.R.UserRoomMessages = append(related.R.UserRoomMessages, o)
	}
...
func RoomMessages(mods ...qm.QueryMod) roomMessageQuery {
	mods = append(mods, qm.From("\"RoomMessage\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"RoomMessage\".*"})
	}
...
func FindRoomMessage(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*RoomMessage, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"RoomMessage\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "sqlboiler: unable to select from RoomMessage")
	}
...
func (o *RoomMessage) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("sqlboiler: no RoomMessage provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"RoomMessage\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"RoomMessage\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to insert into RoomMessage")
	}
...
func (o *RoomMessage) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("sqlboiler: unable to update RoomMessage, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"RoomMessage\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 1, wl),
			strmangle.WhereClause("\"", "\"", len(wl)+1, roomMessagePrimaryKeyColumns),
		)
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to update RoomMessage row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by update for RoomMessage")
	}
...
func (q roomMessageQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to update all for RoomMessage")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to retrieve rows affected for RoomMessage")
	}
...
func (o RoomMessageSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"RoomMessage\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, roomMessagePrimaryKeyColumns, len(o)))
...
func (o *RoomMessage) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoomMessage) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
		return errors.New("sqlboiler: no RoomMessage provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("sqlboiler: unable to upsert RoomMessage, could not build update column list")
		}
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roomMessagePrimaryKeyColumns) == 0 {
				return errors.New("sqlboiler: unable to upsert RoomMessage, could not build conflict column list")
			}

			conflict = make([]string, len(roomMessagePrimaryKeyColumns))
			copy(conflict, roomMessagePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"RoomMessage\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to upsert RoomMessage")
	}
...
func (o *RoomMessage) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"RoomMessage\" WHERE \"id\"=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to delete from RoomMessage")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by delete for RoomMessage")
	}
...
func (q roomMessageQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to delete all from RoomMessage")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by deleteall for RoomMessage")
	}
...
func (o RoomMessageSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"RoomMessage\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, roomMessagePrimaryKeyColumns, len(o))
...
func (o *RoomMessageSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"RoomMessage\".* FROM \"RoomMessage\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, roomMessagePrimaryKeyColumns, len(*o))
...
func RoomMessageExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"RoomMessage\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "sqlboiler: unable to check if RoomMessage exists")
	}
