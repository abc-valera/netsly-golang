...
type userR struct {
	...
	Jokes            JokeSlice        `boil:"Jokes" json:"Jokes" toml:"Jokes" yaml:"Jokes"`
...
func (r *userR) GetJokes() JokeSlice {
	if r == nil {
		return nil
	}
	return r.Jokes
...
func (r *userR) GetRoomMembers() RoomMemberSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetRoomMessages() RoomMessageSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "username", "email", "hashed_password", "fullname", "status", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"id", "username", "email", "hashed_password", "fullname", "status", "created_at", "updated_at", "deleted_at"}
...
// Jokes retrieves all the joke's Jokes with an executor.
func (o *User) Jokes(mods ...qm.QueryMod) jokeQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (o *User) RoomMembers(mods ...qm.QueryMod) roomMemberQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (o *User) RoomMessages(mods ...qm.QueryMod) roomMessageQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"room_messages\".\"user_id\"=?", o.ID),
	)
...
func (o *User) CreatorUserRooms(mods ...qm.QueryMod) roomQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"rooms\".\"creator_user_id\"=?", o.ID),
	)
...
// LoadJokes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadJokes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`jokes`),
		qm.WhereIn(`jokes.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load jokes")
	}

	var resultSlice []*Joke
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice jokes")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on jokes")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for jokes")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Jokes = append(local.R.Jokes, foreign)
				if foreign.R == nil {
					foreign.R = &jokeR{}
				}
				foreign.R.User = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRoomMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`room_members.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRoomMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`room_messages.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadCreatorUserRooms(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`rooms`),
		qm.WhereIn(`rooms.creator_user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddJokesG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Jokes.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddJokesG(ctx context.Context, insert bool, related ...*Joke) error {
	return o.AddJokes(ctx, boil.GetContextDB(), insert, related...)
}

// AddJokes adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Jokes.
// Sets related.R.User appropriately.
func (o *User) AddJokes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Joke) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Jokes = append(o.R.Jokes, related...)
	}
...
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddRoomMembersG(ctx context.Context, insert bool, related ...*RoomMember) error {
...
// Appends related to o.R.RoomMembers.
// Sets related.R.User appropriately.
func (o *User) AddRoomMembers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RoomMember) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
...
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddRoomMessagesG(ctx context.Context, insert bool, related ...*RoomMessage) error {
...
// Appends related to o.R.RoomMessages.
// Sets related.R.User appropriately.
func (o *User) AddRoomMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RoomMessage) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
