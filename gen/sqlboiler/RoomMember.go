...
var RoomMemberTableColumns = struct {
	...
	CreatedAt: "RoomMember.created_at",
...
var RoomMemberWhere = struct {
	...
	CreatedAt: whereHelpertime_Time{field: "\"RoomMember\".\"created_at\""},
	UserID:    whereHelperstring{field: "\"RoomMember\".\"user_id\""},
	RoomID:    whereHelperstring{field: "\"RoomMember\".\"room_id\""},
}
...
var (
	roomMemberAllColumns            = []string{"created_at", "user_id", "room_id"}
	roomMemberColumnsWithoutDefault = []string{"created_at", "user_id", "room_id"}
...
func (q roomMemberQuery) One(ctx context.Context, exec boil.ContextExecutor) (*RoomMember, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "sqlboiler: failed to execute a one query for RoomMember")
	}
...
func (q roomMemberQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to count RoomMember rows")
	}
...
func (q roomMemberQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "sqlboiler: failed to check if RoomMember exists")
	}
...
func (roomMemberL) LoadRoom(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoomMember interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.RoomRoomMembers = append(foreign.R.RoomRoomMembers, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.RoomID == foreign.ID {
				...
				foreign.R.RoomRoomMembers = append(foreign.R.RoomRoomMembers, local)
				break
			}
		}
	}
...
func (roomMemberL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoomMember interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.UserRoomMembers = append(foreign.R.UserRoomMembers, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.UserRoomMembers = append(foreign.R.UserRoomMembers, local)
				break
			}
		}
	}
...
// Adds o to related.R.RoomRoomMembers.
...
func (o *RoomMember) SetRoom(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Room) error {
	...
	if related.R == nil {
		related.R = &roomR{
			RoomRoomMembers: RoomMemberSlice{o},
		}
	} else {
		related.R.RoomRoomMembers = append(related.R.RoomRoomMembers, o)
	}
...
// Adds o to related.R.UserRoomMembers.
// Uses the global database handle.
...
func (o *RoomMember) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	if related.R == nil {
		related.R = &userR{
			UserRoomMembers: RoomMemberSlice{o},
		}
	} else {
		related.R.UserRoomMembers = append(related.R.UserRoomMembers, o)
	}
...
func RoomMembers(mods ...qm.QueryMod) roomMemberQuery {
	mods = append(mods, qm.From("\"RoomMember\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"RoomMember\".*"})
	}
...
func FindRoomMember(ctx context.Context, exec boil.ContextExecutor, userID string, roomID string, selectCols ...string) (*RoomMember, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"RoomMember\" where \"user_id\"=$1 AND \"room_id\"=$2", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "sqlboiler: unable to select from RoomMember")
	}
...
func (o *RoomMember) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("sqlboiler: no RoomMember provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"RoomMember\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"RoomMember\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to insert into RoomMember")
	}
...
func (o *RoomMember) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("sqlboiler: unable to update RoomMember, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"RoomMember\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 1, wl),
			strmangle.WhereClause("\"", "\"", len(wl)+1, roomMemberPrimaryKeyColumns),
		)
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to update RoomMember row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by update for RoomMember")
	}
...
func (q roomMemberQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to update all for RoomMember")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to retrieve rows affected for RoomMember")
	}
...
func (o RoomMemberSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"RoomMember\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, roomMemberPrimaryKeyColumns, len(o)))
...
func (o *RoomMember) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoomMember) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
		return errors.New("sqlboiler: no RoomMember provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("sqlboiler: unable to upsert RoomMember, could not build update column list")
		}
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roomMemberPrimaryKeyColumns) == 0 {
				return errors.New("sqlboiler: unable to upsert RoomMember, could not build conflict column list")
			}

			conflict = make([]string, len(roomMemberPrimaryKeyColumns))
			copy(conflict, roomMemberPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"RoomMember\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to upsert RoomMember")
	}
...
func (o *RoomMember) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"RoomMember\" WHERE \"user_id\"=$1 AND \"room_id\"=$2"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to delete from RoomMember")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by delete for RoomMember")
	}
...
func (q roomMemberQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: unable to delete all from RoomMember")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "sqlboiler: failed to get rows affected by deleteall for RoomMember")
	}
...
func (o RoomMemberSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"RoomMember\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, roomMemberPrimaryKeyColumns, len(o))
...
func (o *RoomMemberSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"RoomMember\".* FROM \"RoomMember\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, roomMemberPrimaryKeyColumns, len(*o))
...
func RoomMemberExists(ctx context.Context, exec boil.ContextExecutor, userID string, roomID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"RoomMember\" where \"user_id\"=$1 AND \"room_id\"=$2 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "sqlboiler: unable to check if RoomMember exists")
	}
